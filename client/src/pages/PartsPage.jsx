        ...
        <div style={{padding:'8px'}}>
          {(form.supplierOptions||[]).map((opt, idx) => (
            <div key={idx} className="row">
              <Select displayEmpty value={opt.supplier} onChange={e=>{
              ...
              <TextField label="Supplier SKU" value={opt.supplierSku||''} onChange={e=>{ const arr=[...form.supplierOptions]; arr[idx]={...arr[idx], supplierSku:e.target.value}; setForm({...form, supplierOptions:arr}) }} />
            </div>
          ))}
        </div>
      </div>)}
