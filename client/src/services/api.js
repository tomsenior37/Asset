...
export async function uploadAssetAttachment(id, file){ const fd = new FormData(); fd.append('file', file); const { data } = await api.post(`/assets/${id}/attachments`, fd, { headers: { 'Content-Type': 'multipart/form-data' } }); return data; }
...
/* Templates / Jobs (if you use them) */
...
export async function applyTemplateToAsset(assetId, { templateId, mode='append' }){ const { data } = await api.post(`/assets/${assetId}/apply-template`, { templateId, mode }); return data; }
export async function cloneBomFromAsset(assetId, { fromAssetId, mode='append' }){ const { data } = await api.post(`/assets/${assetId}/clone-bom`, { fromAssetId, mode }); return data; }

export async function listJobs(assetId){ const { data } = await api.get(`/assets/${assetId}/jobs`); return data; }
export async function createJob(assetId, payload){ const { data } = await api.post(`/assets/${assetId}/jobs`, payload); return data; }
export async function updateJob(id, payload){ const { data } = await api.patch('/jobs/' + id, payload); return data; }
export async function deleteJob(id){ const { data } = await api.delete('/jobs/' + id); return data; }
